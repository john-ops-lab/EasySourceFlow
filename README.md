...
[![Version](https://img.shields.io/badge/version-v0.3.0-FF0000)](CHANGELOG.md)
...
- YouTube Chrome 登录态导入、人工/自动字幕优先级、本地 ASR 回退和可诊断错误。
...
| YouTube | 人工字幕优先、原语言自动字幕次之、本地 ASR 回退 | 支持从本机 Chrome 导入登录态 |
...
处理 YouTube 前，在 Chrome 中登录账号，然后进入“维护 → 账号与授权”接入 Chrome 登录态。接入后，YouTube 任务直接读取所选 Chrome 配置档的当前登录态，避免普通浏览器 Cookie 被轮换后继续使用失效快照；本地过滤文件仅用于状态检查和兼容兜底。
