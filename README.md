...
[![Version](https://img.shields.io/badge/version-v0.8.0-FF0000)](CHANGELOG.md)
...
OpenClaw 用户安装或更新 Skill 后，还需要完成两步：如果修改了 Skill 白名单或 MCP 配置，运行 `openclaw gateway restart`；然后在准备使用的 Agent 聊天中**单独发送 `/new`**。OpenClaw 会复用已有会话的 Skill 快照，只重启网关不一定会让旧聊天采用新规则。
...
- Web 本地文件通过内容上传；Agent 原始附件只允许从明确配置的上传目录读取，默认仅包含 OpenClaw 的标准入站目录。
