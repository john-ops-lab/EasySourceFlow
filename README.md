...
[![Version](https://img.shields.io/badge/version-v0.9.0-FF0000)](CHANGELOG.md)
...
| 飞书等云文档 | Agent 连接器读取完整正文后提交 | 保留原始文档链接并进入结果库 |
...
| 模型列表没有最新型号 | 在“维护 → 模型”填写或选择已保存的 API Key，点击“刷新模型列表”；失败时仍可手工输入官方模型 ID。 |
| 模型测试失败 | 在“维护 → 模型”确认服务商、模型 ID、API Key、余额和 API 地址，再重新测试；失败不会替换当前模型。 |
...
- [OpenClaw](docs/agents/openclaw.md)：使用原生 `mcp add`、`mcp doctor --probe` 和 `skills install`，更新 Skill 后发送 `/new`。
- [Codex](docs/agents/codex.md)：使用 `codex mcp add`，Skill 安装到 `.agents/skills`。
- [Claude Code](docs/agents/claude-code.md)：使用 `claude mcp add`，Skill 安装到 `.claude/skills`。
- [其他 MCP 客户端](docs/agents/generic-mcp.md)：按客户端官方格式配置本地 stdio 服务。
...
完整的通用契约见 [Agent 接入指南](docs/AGENT_INTEGRATION.md)，工具参数见 [MCP API](docs/MCP_API.md)，飞书差异见[飞书适配](docs/channels/feishu.md)。
...
- [OpenClaw 接入](docs/agents/openclaw.md)
- [Codex 接入](docs/agents/codex.md)
- [Claude Code 接入](docs/agents/claude-code.md)
