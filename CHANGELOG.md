...
- 模型维护页支持按服务商保存、选择和删除 API Key，切换服务商时不会错误跳回 DeepSeek 或复用其他服务商凭据。

### 改进

- 结果库在新页面打开，已完成任务可直接打开结果，任务完成后会立即刷新结果入口。
- README 增加可直接执行的安装命令、首次配置顺序、Fake-IP 自检和常见故障排查。
- Fake-IP 命中默认地址池时，任务错误会直接引导到 Web“网络与安全”页面。
