...
1. 检查 `.env`，不要打印 key。
2. 运行健康检查，确认本地配置和依赖完整。
3. 在 Web“模型选择”中手动测试当前模型；该操作会产生一次真实模型请求。
4. 如果只是模型服务暂时失败，稍后重试。
...
如果已经配置两套模型，当前云端模型调用失败后，服务会自动使用标记为“自动备用”的模型重试一次。备用成功时，结果和任务详情记录实际使用的模型以及自动切换状态；主、备用均失败时才使用本地抽取式摘要，并明确写出 `local_extractive_fallback` 和两次失败原因。日志不会记录 API Key。
