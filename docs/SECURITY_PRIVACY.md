...
- 不允许任意文件读写；Agent 附件导入只接受明确配置的上传目录。
- 不允许 URL 或参数触发命令注入。
...
- 不打印 `.env`。
- 不把 key 写进文档、日志或任务结果。
- 自动健康检查只返回本地配置和依赖状态，不返回 key，也不调用外部模型；真实模型测试必须由用户明确触发。
...
`easysourceflow_submit_document_file` 会读取聊天平台保存到本机的原始附件，因此属于额外的本机文件信任边界。
...
飞书等需要登录的云文档由 Agent 已授权的平台连接器读取，再把完整正文通过 MCP 提交给本机服务。EasySourceFlow 不持有平台凭据，也不会重新访问私有文档链接。可选 `source_url` 只接受无凭据的规范 HTTPS URL，用于本地结果的来源标记和缓存隔离；正文、原始链接和生成结果都可能包含私有信息，不得提交到公开仓库或发送到无关服务。
...
云端模型总结会把提取文本发送给配置的第三方模型 API。不要用云端服务处理你不愿发送给第三方的私密内容。

Ollama 和 LM Studio 预设只连接 `127.0.0.1`，请求不会离开本机；用户自行改为局域网或远程兼容地址后，应按远程服务重新评估数据边界和鉴权。
