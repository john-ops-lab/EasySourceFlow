...
### 2.9 `easysourceflow_submit_document`

提交粘贴正文、完整文档文字，或由已认证平台连接器读取到的云文档正文。
...
`source_url` 仅用于连接器已经读取完成的云文档，必须是规范的 HTTPS 链接。服务不会重新访问该地址；它用于保留来源、区分缓存并将结果标记为飞书文档或云文档。普通粘贴文本和本地文档可以省略此字段。

### 2.10 `easysourceflow_submit_document_file`
...
MCP 适配器为向后兼容默认允许 OpenClaw 标准上传目录 `<OPENCLAW_STATE_DIR>/media/inbound`。这不是通用 Agent 路径；其他客户端必须在自己的 MCP 进程环境中设置 `EASYSOURCEFLOW_DOCUMENT_IMPORT_ROOTS`，多个目录使用操作系统路径分隔符连接。文件上限默认 50 MiB，可用 `EASYSOURCEFLOW_DOCUMENT_IMPORT_MAX_BYTES` 调整，最高 200 MiB。路径不在白名单、文件消失、类型不支持或超过上限时，不会读取文件。

### 2.11 `easysourceflow_list_recent_jobs`
...
### 2.12 `easysourceflow_health_check`
...
- GET `/health`。
- 检查输出目录、模型配置完整性、yt-dlp、ffmpeg、whisper、B 站/YouTube cookies、微信浏览器兜底等本地状态。
- 不向模型服务发送请求；真实连通性只通过 Web 中用户明确触发的“测试模型”检查。

### 2.13 `easysourceflow_cleanup`
...
| `POST /model/catalog` | 读取或刷新服务商当前账号可用模型；失败时返回缓存或内置清单 |
